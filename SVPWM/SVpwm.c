...
//  iC[LoopCnt] = Ic;
  ...
  if(cnt_down == cnt40 && epwm_flag == 1)		// uncontrolled rectifier: all IGBTs off
  {
	  cnt_down = 0;
	  epwm_flag = 0;
  ...
  else if(cnt_up == cnt40 && epwm_flag == 0)		// PWM rectifier
  {
	epwm_flag = 1;
	cnt_up = 0;
	...
	// control loop
	Vdc_err = STAND_VD - Ud;
	Idx += Ki_i*Vdc_err + Kp_i*(Vdc_err-Vdc_err_p); Idx = fmod(Idx, 6);
	Vdc_err_p = Vdc_err;

	Id = sqrt23*(Ia-1/2*Ib-1/2*Ic)*cos(phaA)+sqrt2/2*(Ic-Ib)*sin(phaA);
	Iq = -sqrt23*(Ia-1/2*Ib-1/2*Ic)*sin(phaA)+sqrt2/2*(Ic-Ib)*cos(phaA);

	Idy = Idx - Id;
	Iqy = Iqx - Iq;

	uq1 += Ki_u*Idy + Kp_u*(Idy-Idy_p); uq1 = fmod(uq1, 200); Idy_p = Idy;
	uq2 = 100*pi*0.01*Iq;

	ud1 = 100*pi*0.01*Id;
	ud2 += Ki_u*Iqy + Kp_u*(Iqy-Iqy_p); ud2 = fmod(ud2, 200); Iqy_p = Iqy;

	ud = sqrt23*(Uab-1/2*Ubc-1/2*Uca)*cos(theta)+sqrt2/2*(Uca-Ubc)*sin(theta);
	uq = -sqrt23*(Uab-1/2*Ubc-1/2*Uca)*sin(theta)+sqrt2/2*(Uca-Ubc)*cos(theta);
	...
	if(Ud>1)
	{
		v_alpha = (cos(phaA)*vd-sin(phaA)*vq)/(sqrt23*Ud);
		v_beta = (sin(phaA)*vd+cos(phaA)*vq)/(sqrt23*Ud);
	}
	else
	{
		v_alpha = (cos(phaA)*vd-sin(phaA)*vq)/sqrt23;
		v_beta = (sin(phaA)*vd+cos(phaA)*vq)/sqrt23;
	}

	// PWM output
	sector = 0;
	Va = v_beta;
	Vb = -v_beta/2 + 0.8660254*v_alpha;
	Vc = -v_beta/2 - 0.8660254*v_alpha;
	...
	Va = v_beta;
	Vb = 0.5*v_beta + 0.8660254*v_alpha;
	Vc = 0.5*v_beta - 0.8660254*v_alpha;
	if(sector == 0)
	{
		Ta = 0.5;
		Tb = 0.5;
	...
	cmpa = fmod(cmpa,EPWM_PERIOD-1);
	cmpb = fmod(cmpb,EPWM_PERIOD-1);
	cmpc = fmod(cmpc,EPWM_PERIOD-1);
	EPwm1Regs.CMPA.half.CMPA = cmpa;
	EPwm2Regs.CMPA.half.CMPA = cmpb;
	EPwm2Regs.CMPA.half.CMPA = cmpc;
  }
...
void PLL(void)
{
	...
	// ¦Á¦Â transformation & dq transformation
