...
//#define STAND_VD	(200)
#define UNC_VD		(120)	// <= UNC_VD, use uncontrolled rectifier
#define Iqx			(-3)		// variable power factor
// control loop PI
#define Kp_i	0.02
#define Ki_i 	0.00001
#define Kp_u	0.1
...
Uint16 downCnt;
Uint16 upCnt;
Uint16 stableCnt;			// 进入PWM延时1sPI由抗饱和进入饱和
Uint16 stable_flag;			// flag=0, 抗饱和; flag=1, 饱和
...
// voltage external loop global variables
Uint16 sector;
float Vdc_err, Vdc_err_p, Id, Idx, Idy_p, Idy, Iq, Iqy, Iqy_p, ud1, ud2, uq1, uq2, ud, uq;
...
// Step 6. Customlize initialization
   ...
   Iq=0; Iqy=0; Iqy_p=0; ud1=0; ud2=0; uq1=0; uq2=0; ud=0; uq=0;
   Ta=0; Tb=0; Tc=0; t1=0; t2=0; t12=0; vd=0; vq=0; Va=0; Vb=0; Vc=0; v_alpha=0; v_beta=0;
   epwm_flag = 0;
   stableCnt = 0; stable_flag = 0;
...
//	}
  ...
  else if(epwm_flag==1)								// PWM rectifier
  {
	  ...
	  if(stableCnt<10000 && stable_flag == 0)
	  {
		  stableCnt++;
		  stable_flag = 0;
	  }
	...
	// for SPWM
	v_alpha = (cos(phaA)*vd-sin(phaA)*vq);
	v_beta = (sin(phaA)*vd+cos(phaA)*vq);
...
//	}
	...
	Va = v_alpha;
	Vb = -0.5*v_alpha + 0.866025*v_beta;
	Vc = -0.5*v_alpha - 0.866025*v_alpha;
...
void PLL(void)
{
	...
	// 伪尾 transformation & dq transformation
