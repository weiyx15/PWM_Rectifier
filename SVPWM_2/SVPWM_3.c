...
// for GRAPH watching
float Vab[cntMAX],Vbc[cntMAX],Vca[cntMAX],VDD[cntMAX];
float iA[cntMAX],iB[cntMAX],iC[cntMAX];

// voltage external loop global variables
Uint16 sector;
float Vdc_err, Vdc_err_p, Id, Idx, Idy_p, Idy, Iq, Iqx, Iqy, Iqy_p, ud1, ud2, uq1, uq2;
float ud, uq, ud_p, uq_p;
...
// Step 6. Customlize initialization
   ...
   Iq=0; Iqx=0; Iqy=0; Iqy_p=0; ud1=0; ud2=0; uq1=0; uq2=0; ud=0; uq=0; ud_p=0; uq_p=0;
...
//	}
  ...
  if(epwm_flag<2)						// uncontrolled rectifier: all IGBTs off
  {
  ...
  else if(epwm_flag==2)								// PWM rectifier
  {
...
//	}
	...
	// SPWM
	Ta = (float) (2*Va/Ud+0.5);
	Tb = (float) (2*Vb/Ud+0.5);
	Tc = (float) (2*Vc/Ud+0.5);
